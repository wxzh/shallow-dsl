...
classified as a shallow or deep embedding? Some authors working on
...
In this paper, we avoid ambiguitity by using a more precise meaning
...
typically functions, and procedural abstraction being a way to encode
...
The main goal of our paper is to show that OOP languages have
advantages for the implementation of shallow embeddings. 
It is perhaps due to those advantages that some authors 
have considered  {\sc Composite}-based implementations to be deep
embeddings, rather than shallow. However, we argue that the OOP
mechanisms do not change the essence of the implementation, which is
still shallow (i.e. using procedural abstraction). To understand how

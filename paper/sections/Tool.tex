...
@Family interface New extends Base {
	...
	interface Lit {
		default int _eval() { return i(); }
	}
	interface Add {
		default int _eval() {
			return e1()._eval() + e2()._eval();
		}
	}
}
\end{lstlisting} & \begin{tabular}{c}$\Rightarrow$\\Family\\Desugaring\end{tabular} & \begin{lstlisting}
@Obj interface New extends Base {
	@Obj interface Exp extends Base.Exp {
		int _eval();
	}
	@Obj interface Lit extends Exp, Base.Lit {
		int i();
		default int _eval() { return i(); }
	}
	@Obj interface Add extends Exp, Base.Add {
		Exp e1(); Exp e2();
		default int _eval() {
			return e1()._eval() + e2()._eval();
		}
...
interface New extends Base {
	...
	interface Lit extends Exp, Base.Lit {
		int i(); Lit withI(int val);
		default int _eval() { return i(); }
		static Lit of(int _i) {
			return new Lit() {
				int i = _i;
				public int i() { return i; }
				public Lit withI(int val) {
					return Lit.of(val);
				}
			};
		}
	}
	interface Add extends Exp, Base.Add {
		Exp e1(); Exp e2();
		default int _eval() {
			return e1()._eval() + e2()._eval();
		}
		static Add of(Exp _e1, Exp _e2) {
			return new Add() {
				Exp e1 = _e1; Exp e2 = _e2;
...
\end{lstlisting} & \begin{tabular}{c}$\Leftarrow$\\ObjOf\\Desugaring\end{tabular} & \begin{tabular}{l}\begin{lstlisting}
@ObjOf interface New extends Base {
	@ObjOf interface Exp extends Base.Exp {
		int _eval();
	}
	@ObjOf interface Lit extends Exp, Base.Lit {
		int i();
...
\end{lstlisting}\\ \begin{lstlisting}
	...
	@ObjOf interface Add extends Exp, Base.Add {
		Exp e1(); Exp e2();
		default int _eval() {
			return e1()._eval() + e2()._eval();
		}
...
\hline
\end{tabular}
\caption{An example showing the flow of translation. \textcolor{red}{Haoyuan: fields with underscore}}\label{fig:flow}
\end{figure*}
